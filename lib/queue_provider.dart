...
class QueueProvider extends ChangeNotifier {
  ...
  // Sort clients by priority: urgent > vip > normal, then by created_at
  void _sortClientsByPriority() {
    // Convert to mutable list if it's read-only (from sqflite)
    _clients = List<Map<String, dynamic>>.from(_clients);
    _clients.sort((a, b) {
      final priorityA = _getPriorityValue(a['priority'] ?? 'normal');
      final priorityB = _getPriorityValue(b['priority'] ?? 'normal');
      ...
      // If same priority, sort by created_at (earlier first)
